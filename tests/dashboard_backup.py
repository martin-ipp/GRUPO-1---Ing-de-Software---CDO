...
def format_period(p, res):
    if res == "Anual": return str(p.year)
    if res == "Mensual": return f"{meses_es_dict.get(p.month, '')} {p.year}"
    if res == "Semanal": return f"Semana {p.week} ({p.year})"
    if res == "Diaria": return f"{p.day:02d}/{p.month:02d}/{p.year}"

def get_period_series(dates, res):
    if res == "Anual": return dates.dt.to_period('Y')
    if res == "Mensual": return dates.dt.to_period('M')
    if res == "Semanal": return dates.dt.to_period('W')
    return dates.dt.to_period('D')
...
[data-testid="stSidebar"] [role="radiogroup"] label {
    background: #151525 !important; border: 1px solid #2A2A40 !important; border-radius: 8px !important;
    padding: 14px 16px !important; margin: 6px 0 !important; transition: all 0.3s !important; cursor: pointer !important;
...
.main-header {
    ...
    border: 1px solid #1E1E38; border-radius: 12px; padding: 20px 30px; margin-bottom: 20px; position: relative; overflow: hidden;
}
...
.delta-val { font-weight: 600; text-align: right; }
.delta-val.up { color: #00FF88; }
.delta-val.down { color: #FF4757; }
.delta-val.neutro { color: #8888AA; }
...
[data-testid="stMain"] [data-testid="stRadio"] label {
    background: #151525 !important; border: 1px solid #2A2A40 !important; border-radius: 6px !important;
    padding: 6px 14px !important; margin: 0 !important; cursor: pointer !important; transition: all 0.2s;
}
...
/* Forzar ancho y justificación de columnas de radio buttons */
...
.stSelectbox div[data-baseweb="select"] > div:hover { border-color: #00D4FF !important; }
...
@st.cache_data
def get_exact_last_date():
    path = os.path.join(PROCESSED_PATH, "dim_date.csv")
    if os.path.exists(path):
        try:
            df_date = pd.read_csv(path)
            return pd.to_datetime(df_date["date"]).max()
...
df_city_daily = load_csv("ft_city_sales_daily.csv")
df_deep_dive  = load_csv("ft_deep_dive.csv")

if not df_daily.empty: df_daily["date"] = pd.to_datetime(df_daily["date"])
if not df_city_daily.empty: df_city_daily["date"] = pd.to_datetime(df_city_daily["date"])
if not df_deep_dive.empty: df_deep_dive["date"] = pd.to_datetime(df_deep_dive["date"])

if not df_city_daily.empty and not df_daily.empty:
    df_city_counts = df_city_daily.groupby("date")["city"].nunique().reset_index(name="active_cities")
    df_daily = pd.merge(df_daily, df_city_counts, on="date", how="left")
    df_daily["active_cities"] = df_daily["active_cities"].fillna(0)
...
with st.sidebar:
    ...
    exact_date = get_exact_last_date()
    if exact_date is None and not df_daily.empty:
        exact_date = df_daily["date"].max()
        
    if exact_date is not None: last_date_str = f"{exact_date.day} de {meses_es_dict.get(exact_date.month, '')} de {exact_date.year}"
    else: last_date_str = "Fecha Desconocida"
...
if "Resumen" in view and not df_daily.empty:
    ...
    def get_cities(dt): return set(df_city_daily[df_city_daily["date"] == dt]["city"].unique()) if not df_city_daily.empty else set()
    c_0_set, c_1_set, c_m_set, c_y_set = get_cities(t_0), get_cities(t_1), get_cities(m_1), get_cities(y_1)

    def html_geo_delta(current_set, ref_set):
        if not ref_set: return '<td class="delta-val neutro">Sin Historial</td>'
        nuevas, inactivas = len(current_set - ref_set), len(ref_set - current_set)
        html_str = '<td class="delta-val">'
        if nuevas > 0: html_str += f'<span style="color:#00FF88;">+{nuevas} Nuevas</span>'
        if nuevas > 0 and inactivas > 0: html_str += ' | '
        if inactivas > 0: html_str += f'<span style="color:#FF4757;">-{inactivas} Inactivas</span>'
        if nuevas == 0 and inactivas == 0: html_str += '<span style="color:#8888AA;">Sin Cambios</span>'
        html_str += '</td>'
        return html_str
    ...
    with c4:
        ...
        <div class="kpi-card orange" style="{get_card_style('Ciudades', '#FF8800')}">
            ...
            <div class="kpi-value orange">{len(c_0_set)} Ciudades</div>
            <table class="delta-table-mini">
                <tr><td class="delta-lbl">vs Día Ant.</td>{html_geo_delta(c_0_set, c_1_set)}</tr>
                <tr><td class="delta-lbl">vs Mes Ant.</td>{html_geo_delta(c_0_set, c_m_set)}</tr>
                <tr><td class="delta-lbl">vs Año Ant.</td>{html_geo_delta(c_0_set, c_y_set)}</tr>
            </table>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_l:
        ...
        fig_g1 = go.Figure(go.Bar(
            x=df_g1[x1].astype(str) if x1 == "year" else df_g1[x1], y=df_g1[target_col],
            marker=dict(color=df_g1[target_col], colorscale=[[0, "#151525"], [1, color_tema]], line=dict(width=0)),
            text=custom_g1, textposition="outside", textfont=dict(size=11, color=color_tema, family="JetBrains Mono"), 
            customdata=custom_g1, hovertemplate="<b>%{x}</b><br>%{customdata}<extra></extra>", width=0.6 if x1 == "year" else None,
        ))
        fig_g1.update_layout(PLOT_LAYOUT)
        fig_g1.update_layout(height=260, xaxis=dict(type="category" if x1 == "year" else "date"), yaxis=dict(showticklabels=False, showgrid=False))
    ...
    with col_r:
        ...
        fig_g2.update_layout(PLOT_LAYOUT)
        fig_g2.update_layout(height=260, legend=dict(orientation="h", y=1.08, x=0, bgcolor="rgba(0,0,0,0)"))
...
# ══════════════════════════════════════════════
    ...
    if tiempo_dd == "Mensual":
        ...
        df_base = df_base[df_base["date"] >= start_date]
        df_daily_filtered = df_daily_filtered[df_daily_filtered["date"] >= start_date]
...
# ══════════════════════════════════════════════
# VISTA 4 — EXPANSIÓN GEOGRÁFICA
# ══════════════════════════════════════════════
elif "Geográfica" in view and not df_deep_dive.empty:
    ...
    with cont_mid:
        ...
        with c_met:
            ...
            metrica_geo = st.radio("Métrica de Visualización", ["Ingresos", "Volumen", "Ticket"], horizontal=True, key="geo_metric_radio", label_visibility="collapsed")
        
        with c_res:
            st.markdown('<p style="font-size:11px; color:#00D4FF; font-weight:700; margin: 5px 0 5px 0; letter-spacing:0.05em;">⏳ RESOLUCIÓN TEMPORAL</p>', unsafe_allow_html=True)
            res_geo = st.radio("Resolución", ["Anual", "Mensual", "Semanal", "Diaria"], horizontal=True, key="geo_res_radio", label_visibility="collapsed")
            
        per_series_geo = get_period_series(df_geo['date'], res_geo)
        unique_pers_geo = sorted(per_series_geo.dropna().unique(), reverse=True)
        display_opts_geo = {p: format_period(p, res_geo) for p in unique_pers_geo}
        
        with c_per:
            st.markdown('<p style="font-size:11px; color:#00FF88; font-weight:700; margin: 5px 0 5px 0; letter-spacing:0.05em;">📅 PERÍODO A ANALIZAR</p>', unsafe_allow_html=True)
            sel_per_str_geo = st.selectbox("Período a Analizar", list(display_opts_geo.values()), key="geo_per_sel", label_visibility="collapsed")
            
        st.markdown("""<div style="width:100%; height:1px; background:#1A1A2C; margin: 5px 0 15px 0;"></div>""", unsafe_allow_html=True)

    sel_per_geo = [p for p, s in display_opts_geo.items() if s == sel_per_str_geo][0]
    df_geo = df_geo[per_series_geo == sel_per_geo].copy()
    is_curr = (metrica_geo in ["Ingresos", "Ticket"])
    
    df_city_totals = df_geo.groupby(['country', 'city'])[['sales_revenue', 'sales_volume']].sum().reset_index()
    if metrica_geo == "Ticket": df_city_totals['target'] = df_city_totals['sales_revenue'] / df_city_totals['sales_volume']
    else: df_city_totals['target'] = df_city_totals['sales_revenue'] if metrica_geo == "Ingresos" else df_city_totals['sales_volume']
    
    df_country_avg = df_city_totals.groupby('country').agg(avg_val=('target', 'mean'), city_count=('city', 'count')).reset_index()
    df_country_avg = df_country_avg.sort_values('avg_val', ascending=False)
    df_city_totals_sorted = df_city_totals.sort_values('target', ascending=False)

    def get_best_ranked_items(df, group_col, item_col, mode_geo):
        if group_col == 'country':
            agg_city = df.groupby(['country', 'city', item_col])[['sales_revenue', 'sales_volume']].sum().reset_index()
            if mode_geo == "Ticket": agg_city['val'] = agg_city['sales_revenue'] / agg_city['sales_volume']
            else: agg_city['val'] = agg_city['sales_revenue'] if mode_geo == "Ingresos" else agg_city['sales_volume']
            agg = agg_city.groupby(['country', item_col])['val'].mean().reset_index()
        else:
            agg = df.groupby(['city', item_col])[['sales_revenue', 'sales_volume']].sum().reset_index()
            if mode_geo == "Ticket": agg['val'] = agg['sales_revenue'] / agg['sales_volume']
            else: agg['val'] = agg['sales_revenue'] if mode_geo == "Ingresos" else agg['sales_volume']

        agg['rank'] = agg.groupby(item_col)['val'].rank(method='min', ascending=False).astype(int)
        agg_sorted = agg.sort_values([group_col, 'rank', 'val'], ascending=[True, True, False])
        top_items = agg_sorted.groupby(group_col).head(1).copy()
        top_items['display'] = top_items[item_col].astype(str) + " (" + top_items['rank'].astype(str) + "°)"
        return top_items.set_index(group_col)['display'].to_dict()

    top_fam_country = get_best_ranked_items(df_geo, 'country', 'product_family', metrica_geo)
    top_cat_country = get_best_ranked_items(df_geo, 'country', 'product_category', metrica_geo)
    top_fam_city    = get_best_ranked_items(df_geo, 'city', 'product_family', metrica_geo)
    top_cat_city    = get_best_ranked_items(df_geo, 'city', 'product_category', metrica_geo)

    def build_geo_table_v2(df_sorted, top_fam, top_cat, col_geo, r_height, pad_top, w_fam="55%", w_cat="45%"):
        df_rev = df_sorted.iloc[::-1]
        rows = ""
        for _, row in df_rev.iterrows():
            item = row[col_geo]
            f_str = top_fam.get(item, "-")
            c_str = top_cat.get(item, "-")
            f_html = re.sub(r'\((\d+°)\)', r'(<span style="color:#00D4FF; font-weight:700;">\1</span>)', f_str)
            c_html = re.sub(r'\((\d+°)\)', r'(<span style="color:#00FF88; font-weight:700;">\1</span>)', c_str)
        ...
        html = f"""
        <div style="padding-top: {pad_top}px; margin-top: 0px;">
    ...
    with cont_top:
        ...
        with col_header:
            ...
            top_c_str = df_country_avg.iloc[0]['country'] if not df_country_avg.empty else "-"
            top_ci_str = df_city_totals_sorted.iloc[0]['city'] if not df_city_totals_sorted.empty else "-"
        ...
        with col_map:
            df_country_avg["fmt_rev"] = df_country_avg["avg_val"].apply(lambda x: fmt_num(x, is_curr))
            df_country_avg["fmt_cities"] = df_country_avg["city_count"].astype(str) + " tiendas"
            fig_map = px.choropleth(df_country_avg, locations="country", locationmode="country names", color="avg_val", hover_name="country", hover_data={"avg_val": False, "fmt_rev": True, "fmt_cities": True, "country": False}, color_continuous_scale=[[0, "#0A0A18"], [0.2, "#003344"], [0.5, "#006688"], [0.8, "#00AACC"], [1, "#00D4FF"]], labels={"fmt_rev": metrica_geo, "fmt_cities": "Tiendas"})
    ...
    with cont_bot:
        ...
        with col_countries:
            ...
            top10c = df_country_avg.nlargest(10, "avg_val").sort_values("avg_val")
            custom_c = [fmt_num(v, is_curr) for v in top10c["avg_val"]]
            ...
            with cc_right:
                st.markdown("""
                <div class="section-header" style="justify-content: space-between; border-bottom: 1px solid #1A1A2C; padding-bottom: 8px;">
                    <div style="display: flex; align-items: center; gap: 8px; width: 55%;">
                        <div class="section-dot" style="background:#00D4FF;box-shadow:0 0 8px #00D4FF;"></div>
                        <div class="section-title" style="font-size: 14px; font-weight: 700;">FAMILIA</div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px; width: 45%;">
                        ...
                        <div class="section-title" style="font-size: 14px; font-weight: 700;">CATEGORÍA</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                st.markdown(build_geo_table_v2(top10c, top_fam_country, top_cat_country, "country", ALTO_FILA_PAISES, MARGEN_SUPERIOR_TABLA, w_fam="55%", w_cat="45%"), unsafe_allow_html=True)

        with col_cities:
            cci_left, cci_right = st.columns([1.7, 0.9])
            
            if len(df_city_totals_sorted) > 10: top10ci = pd.concat([df_city_totals_sorted.head(5), df_city_totals_sorted.tail(5)])
            else: top10ci = df_city_totals_sorted
            top10ci = top10ci.sort_values("target", ascending=True)
            custom_ci = [fmt_num(v, is_curr) for v in top10ci["target"]]
            ...
            with cci_left:
                ...
                fig_cities = go.Figure(go.Bar(x=top10ci["target"], y=top10ci["city"], orientation="h", marker=dict(color=top10ci["target"], colorscale=[[0, "#1A0033"], [1, "#AA44FF"]], line=dict(width=0)), text=custom_ci, textposition="inside", insidetextanchor="end", textfont=dict(size=11, color="#FFFFFF", family="JetBrains Mono"), customdata=custom_ci, hovertemplate="<b>%{y}</b><br>%{customdata}<extra></extra>"))
            ...
            with cci_right:
                ...
                """, unsafe_allow_html=True)
                st.markdown(build_geo_table_v2(top10ci, top_fam_city, top_cat_city, "city", ALTO_FILA_CIUDADES, MARGEN_SUPERIOR_TABLA, w_fam="45%", w_cat="55%"), unsafe_allow_html=True)

# ══════════════════════════════════════════════
# VISTA 5 — SCORECARD DIRECTIVO
# ══════════════════════════════════════════════
elif "Scorecard" in view and not df_deep_dive.empty:
    ...
    per_series = get_period_series(df_deep_dive['date'], res_score)
    unique_pers = sorted(per_series.dropna().unique(), reverse=True)
    display_opts = {p: format_period(p, res_score) for p in unique_pers}
    ...
    with col_per:
        ...
        sel_per_str = st.selectbox("Período a Analizar", list(display_opts.values()), label_visibility="collapsed")

    st.markdown("""<div style="width:100%; height:1px; background:#1A1A2C; margin: 15px 0 25px 0;"></div>""", unsafe_allow_html=True)

    # --- 2. Cálculos Matemáticos ---
    sel_per = [p for p, s in display_opts.items() if s == sel_per_str][0]
    prev_per = sel_per - 1

    df_curr = df_deep_dive[per_series == sel_per].copy()
    df_prev = df_deep_dive[per_series == prev_per].copy()

    target_col = 'sales_revenue' if metrica_score == 'Ingresos' else 'sales_volume'
    is_curr = (metrica_score == 'Ingresos')

    def get_stats(dim_col, t_col):
        res = {'top_abs_name': '-', 'top_abs_val': 0, 'bot_abs_name': '-', 'bot_abs_val': 0, 'top_g_name': '-', 'top_g_val': np.nan, 'bot_g_name': '-', 'bot_g_val': np.nan}
        ...
        if c_tot.empty and p_tot.empty: return res
        ...
        if df_dim['curr'].max() > 0:
            res['top_abs_name'] = str(df_dim['curr'].idxmax())
            res['top_abs_val'] = df_dim['curr'].max()
            
        curr_active = df_dim[df_dim['curr'] > 0]
        if not curr_active.empty:
            res['bot_abs_name'] = str(curr_active['curr'].idxmin())
            res['bot_abs_val'] = curr_active['curr'].min()
            
        valid_growth = df_dim.dropna(subset=['growth'])
        if not valid_growth.empty:
            res['top_g_name'] = str(valid_growth['growth'].idxmax())
            res['top_g_val'] = valid_growth['growth'].max()
            res['bot_g_name'] = str(valid_growth['growth'].idxmin())
            res['bot_g_val'] = valid_growth['growth'].min()
            
        return res

    dims = [('country', 'País'), ('city', 'Ciudad'), ('product_family', 'Familia'), ('product_category', 'Categoría')]
    results = {name: get_stats(col, target_col) for col, name in dims}

    # --- 3. Renderizado de Filas (Scorecards 5 Columnas) ---
    
    # FILA 1: Top Absoluto
    cols1 = st.columns([1.5, 2, 2, 2, 2])
    with cols1[0]: st.markdown('<div class="kpi-card blue" style="background: #00D4FF15; border: 1px solid #00D4FF50; min-height:95px; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; font-size:12px; font-weight:800; text-transform:uppercase; color:#00D4FF; line-height:1.4;">🏆<br>Mayores<br>Cantidades<br>(Prom. Diario)</div>', unsafe_allow_html=True)
    for i, (_, name) in enumerate(dims):
        res = results[name]
        v_str = f"{fmt_num(res['top_abs_val'], is_curr)}/día" if res['top_abs_name'] != "-" else "-"
        n = res['top_abs_name']
        with cols1[i+1]:
            st.markdown(f'<div class="kpi-card blue" style="min-height:95px;"><div class="kpi-label">Mejor {name}</div><div class="kpi-value blue" style="font-size:24px; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;" title="{n}">{n}</div><div style="text-align:center; color:#A0A0C0; font-family:\'JetBrains Mono\', monospace; font-size:12px; font-weight:600;">{v_str}</div></div>', unsafe_allow_html=True)
    
    st.markdown("<div style='margin-bottom: 25px;'></div>", unsafe_allow_html=True)

    # FILA 2: Top Crecimiento
    cols2 = st.columns([1.5, 2, 2, 2, 2])
    with cols2[0]: st.markdown('<div class="kpi-card green" style="background: #00FF8815; border: 1px solid #00FF8850; min-height:95px; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; font-size:12px; font-weight:800; text-transform:uppercase; color:#00FF88; line-height:1.4;">🚀<br>Mayor<br>Crecimiento</div>', unsafe_allow_html=True)
    for i, (_, name) in enumerate(dims):
        res = results[name]
        v_str = fmt_pct_score(res['top_g_val']) if res['top_g_name'] != "-" else "-"
        n = res['top_g_name']
        with cols2[i+1]:
            st.markdown(f'<div class="kpi-card green" style="min-height:95px;"><div class="kpi-label">Líder en {name}</div><div class="kpi-value green" style="font-size:24px; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;" title="{n}">{n}</div><div style="text-align:center; color:#A0A0C0; font-family:\'JetBrains Mono\', monospace; font-size:12px; font-weight:600;">Crecimiento: {v_str}</div></div>', unsafe_allow_html=True)

    st.markdown("<div style='margin-bottom: 25px;'></div>", unsafe_allow_html=True)

    # FILA 3: Bottom Absoluto
    cols3 = st.columns([1.5, 2, 2, 2, 2])
    with cols3[0]: st.markdown('<div class="kpi-card orange" style="background: #FF880015; border: 1px solid #FF880050; min-height:95px; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; font-size:12px; font-weight:800; text-transform:uppercase; color:#FF8800; line-height:1.4;">⚠️<br>Menores<br>Cantidades<br>(Prom. Diario)</div>', unsafe_allow_html=True)
    for i, (_, name) in enumerate(dims):
        res = results[name]
        v_str = f"{fmt_num(res['bot_abs_val'], is_curr)}/día" if res['bot_abs_name'] != "-" else "-"
        n = res['bot_abs_name']
        with cols3[i+1]:
            st.markdown(f'<div class="kpi-card orange" style="min-height:95px;"><div class="kpi-label">Último en {name}</div><div class="kpi-value orange" style="font-size:24px; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;" title="{n}">{n}</div><div style="text-align:center; color:#A0A0C0; font-family:\'JetBrains Mono\', monospace; font-size:12px; font-weight:600;">{v_str}</div></div>', unsafe_allow_html=True)

    st.markdown("<div style='margin-bottom: 25px;'></div>", unsafe_allow_html=True)

    # FILA 4: Bottom Crecimiento (Caída)
    cols4 = st.columns([1.5, 2, 2, 2, 2])
    with cols4[0]: st.markdown('<div class="kpi-card red" style="background: #FF475715; border: 1px solid #FF475750; min-height:95px; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; font-size:12px; font-weight:800; text-transform:uppercase; color:#FF4757; line-height:1.4;">📉<br>Menor<br>Crecimiento<br>(Caída)</div>', unsafe_allow_html=True)
    for i, (_, name) in enumerate(dims):
        res = results[name]
        v_str = fmt_pct_score(res['bot_g_val']) if res['bot_g_name'] != "-" else "-"
        n = res['bot_g_name']
        with cols4[i+1]:
            st.markdown(f'<div class="kpi-card red" style="min-height:95px;"><div class="kpi-label">Peor {name}</div><div class="kpi-value red" style="font-size:24px; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;" title="{n}">{n}</div><div style="text-align:center; color:#A0A0C0; font-family:\'JetBrains Mono\', monospace; font-size:12px; font-weight:600;">Crecimiento: {v_str}</div></div>', unsafe_allow_html=True)